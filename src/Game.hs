...
gameWithLives numLives level pts =
  dSwitch
...
gamePlay' objs =
  ...
  where
    ...
    adaptInput = arr (\(gi, (os, cs, _)) -> ObjectInput gi cs (map outputObject os))
...
ballBounce' :: SF (ObjectInput, ObjectOutput) (Event (Pos2D, Vel2D))
ballBounce' = proc (ObjectInput _ cs _, o) -> do
...
freeBall p0 v0 = proc (ObjectInput _ cs _) -> do
  -- Detect collisions
  let name = "ball"
...
objPaddle :: ObjectSF
objPaddle = proc (ObjectInput ci cs _) -> do
  -- Detect collisions
  let name = "paddle"
  let isHit = inCollision name cs
...
objBlock ((x, y), initlives) (w, h) = proc (ObjectInput _ cs _) -> do
  -- Detect collisions
...
objWall name side pos = proc (ObjectInput _ cs _) -> do
  let isHit = inCollision name cs
  returnA
    -<
      ObjectOutput
