...
initGraphs = do
  -- Create window
  _ <- SDL.setVideoMode (round width) (round height) 32 [SWSurface]
  SDL.setCaption "Test" ""
...
display resources' shownState = do
  ...
  _ <- SDL.blitSurface hud Nothing screen $ Just rectHud
  ...
  _ <- SDL.blitSurface surface Nothing screen $ Just rect
...
paintGeneralMsg screen resources' = \case
  GamePlaying -> return ()
  GamePaused -> paintGeneralMsg' screen resources' "Paused"
  GameLoading n -> paintGeneralMsg' screen resources' ("Level " ++ show n)
  GameOver -> paintGeneralMsg' screen resources' "GAME OVER!!!"
  GameFinished -> paintGeneralMsg' screen resources' "You won!!! Well done :)"
  _ -> undefined
...
paintGeneralHUD screen resources' over = void $ do
  ...
  _ <- SDL.blitSurface message1 Nothing screen $ Just (SDL.Rect 10 10 w1 h1)
  ...
  _ <- SDL.blitSurface message2 Nothing screen $ Just (SDL.Rect 10 (10 + h2 + 5) w2 h2)
...
paintObject resources' screen object =
  case objectKind object of
    (Paddle (w, h)) -> void $ do
      ...
      _ <- setColorKey bI [SrcColorKey, RLEAccel] t
  ...
  where
    ...
    blockImage _ = block3Img resources'
...
loadResources = runMaybeT $ do
  ...
  res <- case (myFont, blockHit) of
    ...
    _ -> do
      liftIO $ putStrLn "Some resources could not be loaded"
      mzero
...
updateAllResources res (GameLoading n) = do
  ...
  newMusic <-
    if oldMusicFP == newMusicFP
      then return oldMusic
      else do
        -- Loading can fail, in which case we continue
        -- with the old music
        bgM <- loadMusic newMusicFP
        if isNothing bgM
          then do
            putStrLn $ "Could not load resource " ++ newMusicFP
            return oldMusic
          else do
            stopMusic
            return bgM
...
updateAllResources _ _ = undefined
